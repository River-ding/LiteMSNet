...
class DeeplabV3(object):
    _defaults = {
        ...
        #-------------------------------------------------------------------#
        "model_path"        : '/home/dell/experiment/DJ_study/deeplabv3-plus-pytorch-main/logs/best_epoch_weights.pth',
        #----------------------------------------#
        #   所需要区分的类的个数+1
        #----------------------------------------#
        "num_classes"       : 11,
    ...
    def __init__(self, **kwargs):
        ...
        if self.num_classes <= 11:
            self.colors = [(128, 128, 128), (128, 0, 0), (192, 192, 128), (128, 64, 128), (60, 40, 222),
                           (128, 128, 0),
        ...
        for _ in range(test_interval):
            with torch.no_grad():
                ...
                pr = pr[int((self.input_shape[0] - nh) // 2) : int((self.input_shape[0] - nh) // 2 + nh), \
                        int((self.input_shape[1] - nw) // 2) : int((self.input_shape[1] - nw) // 2 + nw)]
        # t2 = time.time()
        tact_time =sum(t_a)/ test_interval
        return tact_time
