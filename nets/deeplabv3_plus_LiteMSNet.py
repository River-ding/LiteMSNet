...
from nets.ARM import AttentionRefinementModule
from nets.DABNet import DABModule
from nets.DAPPM import DAPPM
from nets.attention import PAPPM, ECAAttention, ShuffleAttention, FeatureFusionModule
...
class MobileNetV3(nn.Module):
    def __init__(self, downsample_factor=8, pretrained=True):
        ...
        self.total_idx = len(self.features)
        self.down_idx = [2, 4, 7, 14]
        self.conv1_2 = nn.Conv2d(24, 160, 1, 1, 0, bias=True)
        self.eca = ECAAttention(159)
        self.sa = ShuffleAttention(160)
    ...
    def forward(self, x):  #3,512,512
        ...
        low_level_features = self.sa(low_level_features)
