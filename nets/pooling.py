...
class StripPooling(nn.Module):
    ...
    Reference:
    """
    def __init__(self, in_channels, pool_size, norm_layer=nn.BatchNorm2d, up_kwargs=None):
        ...
        self.conv1_2 = nn.Sequential(nn.Conv2d(in_channels, inter_channels, 1, bias=False),
                                norm_layer(inter_channels),
                                nn.ReLU(True))
        self.conv2_0 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, 3, 1, 1, bias=False),# feature no change
                                norm_layer(inter_channels))
        self.conv2_1 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, 3, 1, 1, bias=False),
                                norm_layer(inter_channels))
        self.conv2_2 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, 3, 1, 1, bias=False),
                                norm_layer(inter_channels))
        self.conv2_3 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, (1, 3), 1, (0, 1), bias=False),#lie padding 1
                                norm_layer(inter_channels))
        self.conv2_4 = nn.Sequential(nn.Conv2d(inter_channels, inter_channels, (3, 1), 1, (1, 0), bias=False),# hang padding 1
                                norm_layer(inter_channels))
    ...
    def forward(self, x):
        _, _, h, w = x.size()
        x1 = self.conv1_1(x)# channel change
        x2 = self.conv1_2(x)# channel change
        x2_1 = self.conv2_0(x1) #3*3conv feature no change
