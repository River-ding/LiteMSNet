...
from nets.GCT import GCT
from nets.PConv import Pconv
from nets.attention import ECAAttention
from nets.doconv import DOConv2d
...
class PBlock(nn.Module):
    def __init__(self,in_channels,out_channels,stride):
        ...
        self.conv2 = Pconv(out_channels, out_channels, kernel_size=3)
        self.bn2 = norm2d(out_channels)
        self.act2 = activation()
        self.conv3 = nn.Conv2d(out_channels, out_channels, kernel_size=1, bias=False)
        self.bn3 = norm2d(out_channels)
        self.act3 = activation()
        self.eca = ECAAttention(out_channels-1)
...
class YBlock(nn.Module):
    def __init__(self, in_channels, out_channels, dilation,group_width, stride):
        ...
        self.gct1 = GCT(in_channels)
        self.conv1=nn.Conv2d(in_channels, out_channels,kernel_size=1,bias=False)
        self.bn1=norm2d(out_channels)
        self.act1=activation()
        self.gct2 = GCT(out_channels)
        ...
        # self.conv2 = DOConv2d(out_channels, out_channels, kernel_size=3, stride=stride, groups=groups,padding=dilation, dilation=dilation, bias=False)
        self.bn2=norm2d(out_channels)
        self.act2=activation()
        self.conv3=nn.Conv2d(out_channels, out_channels,kernel_size=1,bias=False)
        self.bn3=norm2d(out_channels)
